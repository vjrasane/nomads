...
describe('Task', () => {
  ...
  describe('chain', () => {
    ...
    it('chains rejecting promise with rejecting promise', async () => {
      await expect(Task.reject('first').chain(() => Task.reject('second')).fork()).rejects.toBe('first');
    });

    it('chains resolving promise with rejecting promise', async () => {
      await expect(Task.resolve(42).chain(() => Task.reject('second')).fork()).rejects.toBe('second');
    });
  });
  ...
  describe('encase', () => {
    ...
    it('encases chained rejecting promise with rejecting promise', async () => {
      const result = await Task.reject('first').chain(() => Task.reject('second')).encase().fork();
      expect(result.result).toEqual({ tag: 'err', error: 'first' });
    });

    it('encases chained resolving promise with rejecting promise', async () => {
      const result = await Task.resolve(42).chain(() => Task.reject('second')).encase().fork();
