...
export const maybe = <A>(): Optional<Maybe<A>, A> =>
  Optional({
    getOption: (a) => a,
    set: (b, a) => a.map(() => b),
  });

export const record = <R extends Record<any, any>>(k: keyof R): Optional<R, R[keyof R]> =>
  Optional({
    getOption: (a) => Maybe.fromOptional(a[k]),
    set: (b, a) => ({ ...a, [k]: b }),
  });

export const result = <E, A>(): Optional<Result<E, A>, A> =>
  Optional({
    getOption: (a) => a.toMaybe(),
    set: (b, a) => a.map(() => b),
  });

export const fromPrism = <A, B>(prism: Prism<A, B>): Optional<A, B> =>
  Optional({
    getOption: prism.getOption,
    set: prism.reverseGet,
  });

export const fromLens = <A, B>(lens: Lens<A, B>): Optional<A, B> =>
  Optional({
    getOption: (a) => Just(lens.get(a)),
    set: lens.set,
  });

export const nth = <A>(i: number): Optional<Array<A>, A> =>
  Optional({
    getOption: (a) => Maybe.nth(i, a),
    set: (b, a) => [...a].splice(i, 1, b),
  });

export const last = <A>(): Optional<Array<A>, A> =>
  Optional({
    getOption: (a) => Maybe.last(a),
    set: (b, a) => [...a, b],
  });

export const first = <A>(): Optional<Array<A>, A> =>
  Optional({
