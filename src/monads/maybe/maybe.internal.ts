...
export const map = <A, B>(fab: (a: A) => B) => (m: Maybe<A>): Maybe<B> => {
	switch(m.tag) {
		case "just":
...
export const filter = <A>(f: (a: A) => boolean) => (m: Maybe<A>): Maybe<A> => {
	switch(m.tag) {
		case "just":
...
export const fold = <A, B>(fb: () => B, fab: (a: A) => B) => (m: Maybe<A>): B => {
	switch(m.tag) {
		case "just":
			return fab(m.value)
		default:
...
export const defaultTo = <A>(def: A) => (m: Maybe<A>): Maybe<A> => {
	switch(m.tag) {
		case "just":
			return m;
		default:
...
export const getOrElse = <A>(def: A) => (m: Maybe<A>): A => {
	switch(m.tag) {
		case "just":
			return m.value;
		default:
...
export const toResult = <E, A>(err: E) => (m: Maybe<A>): Result<E, A> => {
	switch(m.tag) {
		case "just":
			return Ok(m.value);
		default:
