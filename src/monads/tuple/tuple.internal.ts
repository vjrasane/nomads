...
export const mapFirst = <A, B, C>(fac: (a: A) => C) => (t: Tuple<A, B>): Tuple<C, B> => Tuple(fac(t.first), t.second);

export const mapSecond = <A, B, C>(fbc: (b: B) => C) => (t: Tuple<A, B>): Tuple<A, C> => Tuple(t.first, fbc(t.second));

export const mapBoth = <A, B, C, D>(fac: (a: A) => C, fbd: (b: B) => D) => (t: Tuple<A, B>): Tuple<C, D> => Tuple(fac(t.first), fbd(t.second));

export const swap = <A, B>(t: Tuple<A, B>): Tuple<B, A> => Tuple(t.second, t.first);

export const fold = <A, B, C>(fabc: (a: A, b: B) => C) => (t: Tuple<A, B>): C => fabc(t.first, t.second);
